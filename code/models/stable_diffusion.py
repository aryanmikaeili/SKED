...
from utils.general import latent2rgb
...
class StableDiffusion(nn.Module):
    def __init__(self, device, model_name='CompVis/stable-diffusion-v1-4',concept_name=None):
        super().__init__()
    ...
    def produce_latents(self, text_embeddings, height=512, width=512, num_inference_steps=50, guidance_scale=7.5, latents=None):
        ...
        with torch.autocast('cuda'):
            for i, t in enumerate(self.scheduler.timesteps):
                ...
                rgb_latents = latent2rgb(latents)
                Image.fromarray((rgb_latents.squeeze(0).clamp(0, 1).permute(1, 2, 0).cpu().numpy() * 255.).astype('uint8')).save('test_{}.png'.format(i))
                # a = 0
